...
async function handleContact(context) {
    ...
    const endpoint = `https://formsubmit.co/ajax/${encodeURIComponent(context.env.CONTACT_EMAIL)}`;
    const response = await fetch(endpoint, {
        ...
        body: JSON.stringify({
            ...payload,
            _subject: `ExamPortal Contact: ${payload.subject}`,
            _template: "table"
        })
    });
    const result = await response.json().catch(() => ({}));
    if (!response.ok || !(result.success === true || result.success === "true")) {
        console.error("Contact upstream failure", response.status, result.message || "Unknown error");
