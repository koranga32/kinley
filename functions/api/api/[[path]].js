...
import {
    ...
    validateAdminQuestionDeletePayload,
...
const POLICIES = {
    ...
    "admin-question-delete": { windowMs: 10 * MINUTE, ipLimit: 60, sessionLimit: 60 },
...
async function handleAdminQuestionDelete(context) {
    if (context.request.method !== "POST") return methodNotAllowed(["POST"]);
    const authFailure = await requireAdminAccess(context);
    if (authFailure) return authFailure;
    const payload = validateAdminQuestionDeletePayload(await readJson(context.request, 4096));
    await supabaseServerRequest(context.env, `${payload.table}?id=eq.${encodeURIComponent(payload.id)}`, {
        method: "DELETE",
        prefer: "return=minimal"
    });
...
export async function onRequest(context) {
    ...
    try {
        ...
        else if (name === "admin-question-delete") response = await handleAdminQuestionDelete(context);
